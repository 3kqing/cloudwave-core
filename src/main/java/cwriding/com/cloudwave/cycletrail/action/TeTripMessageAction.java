...
import com.cloudwave.cycletrail.service.TeAtUserService;
import com.cloudwave.cycletrail.service.TeLocationService;
import com.cloudwave.cycletrail.service.TeTravelMessageService;
...
import com.cloudwave.trailends.entity.TripMessageBean;
...
public class TeTripMessageAction extends AbstractAction {
	//定义一个全局的记录器，通过LoggerFactory获取
    private final static Logger logger = LoggerFactory.getLogger(TeTripMessageAction.class); 
    
	@Resource
	private TeTravelMessageService teTravelMessageService;
	@Resource
	private UserService userService;
	@Resource
	private TeAtUserService teAtUserService;
	@Resource
	private TeLocationService teLocationService;
	@Resource
	private FileEntityService fileEntityService;
	
	@ResponseBody
	@RequestMapping(method=RequestMethod.POST, value="/save")
	public ResponseEntity save(@RequestParam("file") MultipartFile file, @Valid TripMessageBean tmb, BindingResult result) {
		ResponseEntity re = new ResponseEntity();
		if (result.hasErrors()) {
			re.setCode(ResponseEntity.ERROR, "数据绑定错误!");
			return re;
		}
		ResultEntity r = tmb.toTripMessage();
		if (ResultEntity.FAIL.equals(r.getCode())) {
			return re.parseResult(r);
		}
		CtTripMessage tm = (CtTripMessage) r.getData();
...
//			return re;
		...
		// 处理at的问题
		List<Long> idList = TripMessageBean.convertToIdList(tmb.getAt(), ";");
		List<User> uList = null;
		if (CollectionUtils.isNotEmpty(idList)) {
			uList = this.userService.findByIds(idList);
		}
		// 处理发送到微博的问题
...
//		}
		...
		try {
			this.teTravelMessageService.save(tm);
			
			if (tm.getLocation() != null) {  //保存详细位置信息
				this.teLocationService.save(tm.getLocation());
			}
			
			if (CollectionUtils.isNotEmpty(uList)) {  //保存AT关联用户
				List<CtAtUser> atuList = new ArrayList<CtAtUser>();
				for (User u : uList) {
					CtAtUser au = new CtAtUser(tm.getId(), u.getId());
					atuList.add(au);
				}
				this.teAtUserService.saveAll(atuList);
			}
		} catch (Exception e) {
	...
	public ResponseEntity get(@PathVariable("id") Long id) {
		...
		CtTripMessage tm = this.teTravelMessageService.get(id);
		if (tm != null) {
	...
	public ResponseEntity loadNewer(@RequestParam("id") Long id) {
		...
		if (id == null) {
			tmList = teTravelMessageService.loadNewest(StaticsVariable.ONCE_REQEST_COUNT);
		} else {
			tmList = this.teTravelMessageService.loadNewer(StaticsVariable.ONCE_REQEST_COUNT, id);
		}
	...
	public ResponseEntity loadOlder(@RequestParam("id") Long id) {
		...
		List<CtTripMessage> tmList = null;
		if (id != null) {
			tmList = this.teTravelMessageService.loadOrder(StaticsVariable.ONCE_REQEST_COUNT, id);
		} else {
			tmList = this.teTravelMessageService.loadNewest(StaticsVariable.ONCE_REQEST_COUNT);
		}
